...
if inp:
    ...
    # found data
    # st.success(f"Data found for {inp}")
...
#         df["MA_ONE"] = df["Close"].rolling(window=ma_one if ma_one else 50).mean()
#         df["MA_TWO"] = df["Close"].rolling(window=ma_two if ma_two else 200).mean()
